...
#define EXAMPLE_PIN_NUM_LCD_SCLK           14
#define EXAMPLE_PIN_NUM_LCD_MOSI           13
#define EXAMPLE_PIN_NUM_LCD_MISO           12
...
static void example_lvgl_touch_cb(lv_indev_drv_t * drv, lv_indev_data_t * data)
{
    ...
    bool touchpad_pressed = esp_lcd_touch_get_coordinates(drv->user_data, touchpad_x, touchpad_y, NULL, &touchpad_cnt, 1);
        ...
        x = ((x - min_x) * EXAMPLE_LCD_H_RES) / (max_x - min_x);
        y = ((y - min_y) * EXAMPLE_LCD_V_RES) / (max_y - min_y);
    #endif

    if(touchpad_pressed){
        ESP_LOGI(TAG, "Touchpad: %dx%d -> %d", x, y, touchpad_pressed);
    }
...
void app_main(void)
{
    ...
    spi_bus_config_t buscfg = {
        .sclk_io_num = EXAMPLE_PIN_NUM_LCD_SCLK,
